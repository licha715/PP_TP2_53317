# PP\_TP2\_53317 — Paradigmas de Programación
...
1. `File` → `New` → `Project from Existing Sources...` y elegí la carpeta `PP\_TP2\_53317`.
...
Ya se compiló y ejecutó para verificar que no tenga errores — el archivo `salida\_consola.txt` incluido tiene una corrida completa de ejemplo. Para la entrega necesitás una **captura de pantalla** (imagen) de una ejecución tuya desde IntelliJ, no el `.txt`.

> Nota sobre acentos: si al correrlo desde una terminal ves símbolos raros en vez de tildes (`?` en lugar de `á`), es un tema de la codificación de esa terminal, no del código — en IntelliJ se va a ver bien porque usa UTF-8 por defecto.
